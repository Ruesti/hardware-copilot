...
def build_pcb(model: ExportModel, out_path: Path) -> list[dict]:
    ...
    from .guide import project_hints
    hint_x = max_x + 8.0
    hint_y = ROW_TOP - 10.0
    texts.append({"text": "Routing-Hinweise (Layer Cmts.User, ausblendbar):",
                  "x": hint_x, "y": hint_y, "layer": "comments"})
    for n, hint in enumerate(project_hints(model), start=1):
        texts.append({"text": f"{n}. {hint}", "x": hint_x,
                      "y": hint_y + 5.0 * n, "layer": "comments"})
